...
if(isset($_SESSION['user_id'])) {
    ...
    $userRole = $_SESSION["userrole"]; // You should fetch this from the database or session

    // Display content based on the user's role

    if($userRole !== "superadmin") {
        header("Location: unauthorized.php"); // Create an unauthorized page
        exit();
    }

    // Display content for super admin
    echo "<h1>Welcome Super Admin!</h1>";
    // Display links to super admin specific actions

    // Rest of your code for displaying user details
} else {
...
// Pagination
$tblusersPerPage = 5; // Number of records to display per page
$page = isset($_GET['page']) ? $_GET['page'] : 1;
$offset = ($page - 1) * $tblusersPerPage;

// Fetch records for the current page
 $sql = "SELECT * FROM tblusers LIMIT $offset, $tblusersPerPage";
...
<!--Fetch the Records -->
                    <tr>
                        <td><?php echo $cnt;?></td>
                        <td><img src="propics/<?php  echo $row['ProfilePic'];?>" width="80" height="80"></td>
